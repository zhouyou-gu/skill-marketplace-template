...
- If present, `REVISION_TASK.md` is an auxiliary task brief for joint manuscript and response-letter work. It sits outside the four-file control system, does not change the mandatory read order of the core files, and cannot override them.
...
`REVISION_TASK.md` is optional. It may exist when the workspace needs a dedicated brief for joint manuscript and response-letter revision or drafting.

- **Holds:** the current joint revision package, artifact mapping, review-item routing, manuscript-response synchronization rules, revision-highlighting rules, and response-letter drafting guidance specific to the current revision effort.
- **Precedence:** it lies outside the core four-file control-system precedence. If it conflicts with `AGENT.md`, `AGENT_GOAL.md`, `AGENT_HARNESS.md`, or `AGENT_PROGRESS.md`, the core file wins.
- **Read order:** when present and relevant to the active workstream, read it after `AGENT_PROGRESS.md`. Its existence does not change the mandatory read order of the core four files.
- **Update trigger:** update it in the same turn when the joint revision packet, artifact mapping, or response-letter drafting task changes.
- **Prohibited from:** redefining the mission, storing durable reusable workflow policy that belongs in `AGENT_HARNESS.md`, or replacing `AGENT_PROGRESS.md` as the canonical live-state ledger.
...
If `REVISION_TASK.md` exists and the active workstream involves joint manuscript and response-letter drafting, read it only after the four-file sequence above is complete.
...
- consult `REVISION_TASK.md`, when present and relevant, for the current joint manuscript-response brief without treating it as a control authority;
