...
- Do not use this skill to scaffold AGENT files. If `AGENT_GOAL.md` does not exist, route to `agent-files`.
...
- `research_brief`: concise statement of the research intent
- `chat_context`: optional conversation excerpts, notes, or prior wording
- `dry_run`: when true, return a structured preview without writing

## Workflow

1. Verify that `target_dir/AGENT_GOAL.md` exists. If it does not, stop and tell the caller to scaffold the workspace with `agent-files` first.
...
3. Read `research_brief` and any `chat_context`, then interact with the user until the research intention is explicit enough to save.
4. When the user has already approved exact wording, preserve that wording unless the user explicitly asks for a revision.
5. Curate the whole `AGENT_GOAL.md` so the sections are mutually consistent and reflect the same research intention.
...
- If `AGENT_GOAL.md` is missing, fail cleanly and route to `agent-files`.
